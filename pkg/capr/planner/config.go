...
func addUserConfig(config map[string]interface{}, controlPlane *rkev1.RKEControlPlane, entry *planEntry) error {
	...
	if windows(entry) && controlPlane.Spec.WindowsDataDirectories.K8sDistro != "" {
		config["data-dir"] = capr.GetWindowsDistroDataDir(controlPlane)
	} else if controlPlane.Spec.DataDirectories.K8sDistro != "" {
...
func getMachineNetworkInfo(secrets corecontrollers.SecretCache, entry *planEntry) (*machineNetworkInfo, error) {
	...
	if entry.Machine.Spec.InfrastructureRef.APIGroup != capr.RKEMachineAPIGroup || (internalProvided && externalProvided) {
		return info, nil
	}
